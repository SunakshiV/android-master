...
//        setContentView(R.layout.my_cropyme);
    ...
    private void startGetKbtNoticeCall() {
        com.procoin.http.util.CommonUtil.cancelCall(getKbtNoticeCall);
    ...
    private void startKbtRepoCall(String amount) {
        ...
        com.procoin.http.util.CommonUtil.cancelCall(kbtRepoCall);
        showProgressDialog();
    ...
    private void startGetKbtRepoHome() {
        com.procoin.http.util.CommonUtil.cancelCall(getKbtRepoHomeCall);
        ...
        switch (v.getId()) {
            case R.id.tvBuyBack:
                if (isLogin()) {
                    if (kbtPool == null) return;
                    if (!isOpenBuy) {
                        com.procoin.http.util.CommonUtil.showmessage("暂未开放", this);
                        return;
                    }
                    if (TextUtils.isEmpty(kbtPool.symbol) && kbtPool.subId == 0) {
                        com.procoin.http.util.CommonUtil.showmessage("当前暂无认购", this);
                        return;
                    }
